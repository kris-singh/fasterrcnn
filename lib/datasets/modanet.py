...
class modanet(imdb):
    def __init__(self, image_set):
        imdb.__init__(self, 'modanet_' + image_set)
        # COCO specific config options
        self.config = {'use_salt': True, 'cleanup': True}

        self._image_set = image_set
        self._data_path = osp.join(cfg.DATA_DIR, 'modanet')
        ...
        self._view_map = {'modanet': 'modanet', 'modanetval': 'modanetval'
    ...
    def _get_ann_file(self):
        prefix = 'instances' if self._image_set.find('val') == -1 else 'image_info'
        if self._image_set.find('val') == -1:
            return osp.join(self._data_path, 'annotations', 'modanet2018_instances_train' + '.json')
        else:
            return osp.join(self._data_path, 'annotations', 'modanet2018_instances_val' + '.json')
    ...
    def _load_image_set_index(self):
        ...
        # Code for removing the annotations which have no images
        idxs = []
        for index in image_ids:
            file_name = (str(index).zfill(7) + '.jpg')
            image_path = osp.join(self._data_path, 'images', self._data_name, file_name)
            if not osp.exists(image_path):
                idxs.append(index)
        image_ids = list(set(image_ids).difference(set(idxs)))
        return image_ids
    ...
    def _load_coco_annotation(self, index):
        ...
        return {'width': width, 'height': height, 'boxes': boxes, 'gt_classes': gt_classes, 'gt_overlaps': overlaps,
    ...
    def append_flipped_images(self):
        ...
        for i in range(num_images):
            ...
            entry = {'width': widths[i], 'height': self.roidb[i]['height'], 'boxes': boxes,
                     'gt_classes': self.roidb[i]['gt_classes'], 'gt_overlaps': self.roidb[i]['gt_overlaps'],
                     'flipped': True, 'seg_areas': self.roidb[i]['seg_areas']}
    ...
    def _coco_results_one_category(self, boxes, cat_id):
        ...
        for im_ind, index in enumerate(self.image_index):
            ...
            hs = dets[:, 3] - ys + 1
            results.extend(
                [{'image_id': index, 'category_id': cat_id, 'bbox': [xs[k], ys[k], ws[k], hs[k]], 'score': scores[k]}
